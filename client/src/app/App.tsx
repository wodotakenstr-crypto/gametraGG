...
function MonthlyCloseControl({ closures, dailyArchives, onClose }: { closures: MonthlyClosure[]; dailyArchives: DailyArchive[]; onClose: () => void }) {
  ...
  return <>{header && createPortal(<><button className="daily-date-button" type="button" onClick={() => setShowDates((visible) => !visible)}><Icon name="calendar" size={16} /> {currentDateShort()} <Icon name="chevron" size={14} /></button><button className="monthly-close-button" type="button" onClick={onClose}>Cerrar mes · día 5</button>{showDates && <div className="daily-date-menu">{dailyArchives.length ? dailyArchives.slice().reverse().map((archive) => <button type="button" key={archive.id} onClick={() => { setSelectedArchive(archive); setShowDates(false); }}>{archive.date}<span>{archive.orders.length} pedidos</span></button>) : <p>Aún no hay días archivados.</p>}</div>}</>, header)}{reports && (closures.length > 0 || dailyArchives.length > 0) && createPortal(<div className="monthly-closures"><p className="section-kicker">HISTORIAL DE REPORTES</p>{dailyArchives.length > 0 && <><h3>Reportes diarios</h3>{dailyArchives.slice().reverse().map((archive) => <div key={archive.id}><b>{archive.date}</b><span>{archive.orders.length} pedidos · {money(archive.orders.reduce((sum, order) => sum + order.total, 0))} <button type="button" onClick={() => setSelectedArchive(archive)}>Ver registro</button><button type="button" onClick={() => downloadArchive(archive)}>Descargar</button></span></div>)}</>}{closures.length > 0 && <><h3>Cierres mensuales</h3>{closures.map((closure) => <div key={closure.id}><b>{closure.period}</b><span>{closure.orders.length} pedidos · {money(closure.orders.reduce((sum, order) => sum + order.total, 0))}</span></div>)}</>}</div>, reports)}{selectedArchive && createPortal(<div className="daily-report-modal" role="dialog" aria-modal="true"><div><button className="daily-report-close" type="button" onClick={() => setSelectedArchive(null)}>×</button><p className="section-kicker">REGISTRO DIARIO</p><h2>{selectedArchive.date}</h2><p>{selectedArchive.orders.length} pedidos · Ventas: {money(selectedArchive.orders.reduce((sum, order) => sum + order.total, 0))}</p><div className="daily-report-table">{selectedArchive.orders.length ? selectedArchive.orders.map((order) => <div key={order.id}><b>{order.client}</b><span>{getOrderItems(order).map((item) => `${item.quantity} × ${item.product}`).join(", ")}</span><em>{order.payment} · {money(order.total)}</em></div>) : <p>No hubo pedidos ese día.</p>}</div><button className="daily-download" type="button" onClick={() => downloadArchive(selectedArchive)}>Descargar registro</button></div></div>, document.body)}</>;
}
...
export function App() {
  ...
  return <div className={`water-app ${activeSection === "Repartidor" ? "rider-mode" : ""}`}>
    ...
    <main className="workspace" id="top">
     ...
     {activeSection === "Reportes" && <MonthlyCloseControl closures={monthlyClosures} dailyArchives={dailyArchives} onClose={closeMonthlyPeriod} />}
