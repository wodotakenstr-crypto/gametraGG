...
function InventoryEditors({ inventory, onChange, onPriceChange, onDelete, onEnableProduct, onToggleUnlimited }: { inventory: InventoryItem[]; onChange: (id: string, stock: number) => void; onPriceChange: (id: string, price: number) => void; onDelete: (item: InventoryItem) => void; onEnableProduct: (id: string) => void; onToggleUnlimited: (id: string) => void }) {
  ...
  return <>{inventory.map((item, index) => slots[index] && createPortal(<div className="inventory-controls"><label>Precio<input className="inventory-price-input" aria-label={`Precio de ${item.name}`} type="number" min="0" value={item.price ?? 0} onChange={(event) => onPriceChange(item.id, Number(event.target.value))} /></label>{isUnlimitedProduct(item.name) ? <span className="inventory-unlimited">Stock ilimitado</span> : <label>Stock<input className="inventory-stock-input" aria-label={`Cantidad de ${item.name}`} type="number" min="0" value={item.stock} onChange={(event) => onChange(item.id, Number(event.target.value))} /></label>}<div className="inventory-control-buttons"><label className="inventory-unlimited-toggle"><input type="checkbox" checked={isUnlimitedProduct(item.name)} onChange={() => onToggleUnlimited(item.id)} /> Ilimitado</label><button type="button" className="inventory-order-button" disabled={item.category !== "Productos" || item.availableForSale !== false} onClick={() => onEnableProduct(item.id)}>{item.availableForSale === false ? "Pasar a pedidos" : "En pedidos"}</button><button type="button" className="inventory-delete" aria-label={`Eliminar ${item.name}`} onClick={() => onDelete(item)}>Eliminar</button></div></div>, slots[index], item.id))}</>;
}
