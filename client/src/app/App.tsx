...
type DailyArchive = { id: string; date: string; archivedAt: string; orders: Order[]; expenses: { name: string; value: number }[] };
...
const removedProductNames = new Set(["Bidón", "Dispensador", "Sellos termoencogibles"]);
...
const inventoryOnlyProducts: InventoryItem[] = [
  ...
  { id: "tapas-seguridad", name: "Tapas de seguridad", category: "Insumos", stock: 1000, unit: "unidades", minimum: 100, price: 0 },
];
...
const numberedDestinationIcon = (number: number) => divIcon({ className: "", html: `<span class="route-number-pin"><b>${number}</b></span>`, iconSize: [34, 34], iconAnchor: [17, 34] });
...
function LiveRouteMap({ driverLocation, nextStop, stops = [] }: { driverLocation: DriverLocation; nextStop?: Order; stops?: Order[] }) {
  ...
  const destinationPoints = destinations.map((stop, index) => { const point = comunaCoordinates[stop.comuna] ?? [-33.456, -70.598] as [number, number]; return [point[0] + ((index % 3) - 1) * 0.008, point[1] + Math.floor(index / 3) * 0.008] as [number, number]; });
...
function InventoryEditors({ inventory, onChange, onPriceChange, onDelete, onEnableProduct, onToggleUnlimited }: { inventory: InventoryItem[]; onChange: (id: string, stock: number) => void; onPriceChange: (id: string, price: number) => void; onDelete: (item: InventoryItem) => void; onEnableProduct: (id: string) => void; onToggleUnlimited: (id: string) => void }) {
  const [slots, setSlots] = useState<HTMLElement[]>([]);
  useEffect(() => {
    const frame = requestAnimationFrame(() => setSlots(Array.from(document.querySelectorAll<HTMLElement>(".inventory-list article")).map((card) => {
  ...
  return <>{inventory.map((item, index) => slots[index] && createPortal(<div className="inventory-controls"><label>Precio<input className="inventory-price-input" aria-label={`Precio de ${item.name}`} type="number" min="0" value={item.price ?? 0} onChange={(event) => onPriceChange(item.id, Number(event.target.value))} /></label>{isUnlimitedProduct(item.name) ? <span className="inventory-stock-placeholder" /> : <label>Stock<input className="inventory-stock-input" aria-label={`Cantidad de ${item.name}`} type="number" min="0" value={item.stock} onChange={(event) => onChange(item.id, Number(event.target.value))} /></label>}<label className="inventory-unlimited-toggle"><input type="checkbox" checked={isUnlimitedProduct(item.name)} onChange={() => onToggleUnlimited(item.id)} /> Ilimitado</label><div className="inventory-control-buttons"><button type="button" className="inventory-order-button" disabled={item.category !== "Productos" || item.availableForSale !== false} onClick={() => onEnableProduct(item.id)}>{item.availableForSale === false ? "Pedidos" : "En pedidos"}</button><button type="button" className="inventory-delete" aria-label={`Eliminar ${item.name}`} onClick={() => onDelete(item)}>×</button></div></div>, slots[index], item.id))}</>;
}
...
function OrderProductEditors({ products, cartItems, onAdd, onPriceChange, onNameChange, onRemove, onReorder }: { products: ProductOption[]; cartItems: OrderItem[]; onAdd: (product: ProductOption) => void; onPriceChange: (name: string, price: number) => void; onNameChange: (name: string, nextName: string) => void; onRemove: (name: string) => void; onReorder: (from: string, to: string) => void }) {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      const cards = Array.from(document.querySelectorAll<HTMLElement>(".order-product-editor"));
      cards.forEach((card, index) => {
        card.draggable = true;
        card.ondragstart = (event) => event.dataTransfer?.setData("text/plain", products[index]?.name ?? "");
        card.ondragover = (event) => event.preventDefault();
        card.ondrop = (event) => { event.preventDefault(); const from = event.dataTransfer?.getData("text/plain"); const to = products[index]?.name; if (from && to && from !== to) onReorder(from, to); };
      });
    });
    return () => cancelAnimationFrame(frame);
  }, [products, onReorder]);
...
function MonthlyCloseControl({ closures, dailyArchives, onClose, onDailyClose }: { closures: MonthlyClosure[]; dailyArchives: DailyArchive[]; onClose: () => void; onDailyClose: () => void }) {
  ...
  function downloadArchive(archive: DailyArchive) { const blob = new Blob([JSON.stringify(archive, null, 2)], { type: "application/json" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `reporte-${archive.date}.json`; link.click(); URL.revokeObjectURL(link.href); }
  async function shareArchive(archive: DailyArchive) { const totals = archiveTotals(archive); const text = `Reporte De La Roca ${archive.date}\n${archive.orders.length} pedidos\nVentas: ${money(totals.sales)}\nGastos: ${money(totals.expenses)}\nTotal neto: ${money(totals.net)}`; if (navigator.share) await navigator.share({ title: `Reporte ${archive.date}`, text }); else { await navigator.clipboard?.writeText(text); window.alert("Resumen copiado. Puedes pegarlo en WhatsApp."); } }
  return <>{header && createPortal(<><button className="daily-date-button" type="button" onClick={() => setShowDates((visible) => !visible)}><Icon name="calendar" size={16} /> {currentDateShort()} <Icon name="chevron" size={14} /></button><button className="daily-close-button" type="button" onClick={onDailyClose}>Cerrar día y guardar</button><button className="monthly-close-button" type="button" onClick={onClose}>Cerrar mes · día 5</button>{showDates && <div className="daily-date-menu">{dailyArchives.length ? dailyArchives.slice().reverse().map((archive) => <button type="button" key={archive.id} onClick={() => { setSelectedArchive(archive); setShowDates(false); }}>{archive.date}<span>{archive.orders.length} pedidos</span></button>) : <p>Aún no hay días archivados.</p>}</div>}</>, header)}{reports && (closures.length > 0 || dailyArchives.length > 0) && createPortal(<div className="monthly-closures"><p className="section-kicker">HISTORIAL DE REPORTES</p>{dailyArchives.length > 0 && <><h3>Reportes diarios</h3>{dailyArchives.slice().reverse().map((archive) => <div key={archive.id}><b>{archive.date}</b><span>{archive.orders.length} pedidos · {money(archive.orders.reduce((sum, order) => sum + order.total, 0))} <button type="button" onClick={() => setSelectedArchive(archive)}>Ver registro</button><button type="button" onClick={() => void shareArchive(archive)}>Compartir</button><button type="button" onClick={() => downloadArchive(archive)}>Descargar</button></span></div>)}</>}{closures.length > 0 && <><h3>Cierres mensuales</h3>{closures.map((closure) => <div key={closure.id}><b>{closure.period}</b><span>{closure.orders.length} pedidos · {money(closure.orders.reduce((sum, order) => sum + order.total, 0))}</span></div>)}</>}</div>, reports)}{selectedArchive && createPortal(<div className="daily-report-modal" role="dialog" aria-modal="true"><div><button className="daily-report-close" type="button" onClick={() => setSelectedArchive(null)}>×</button><p className="section-kicker">REGISTRO DIARIO</p><h2>{selectedArchive.date}</h2><p>{selectedArchive.orders.length} pedidos · Ventas: {money(selectedArchive.orders.reduce((sum, order) => sum + order.total, 0))}</p><div className="daily-report-table">{selectedArchive.orders.length ? selectedArchive.orders.map((order) => <div key={order.id}><b>{order.client}</b><span>{getOrderItems(order).map((item) => `${item.quantity} × ${item.product}`).join(", ")}</span><em>{order.payment} · {money(order.total)}</em></div>) : <p>No hubo pedidos ese día.</p>}</div><button className="daily-download" type="button" onClick={() => void shareArchive(selectedArchive)}>Compartir registro</button><button className="daily-download" type="button" onClick={() => downloadArchive(selectedArchive)}>Descargar registro</button></div></div>, document.body)}</>;
}

function ArchivedReportEditor({ archives, onUpdate }: { archives: DailyArchive[]; onUpdate: (archive: DailyArchive) => void }) {
  const [container, setContainer] = useState<HTMLElement | null>(null);
  const [draft, setDraft] = useState<DailyArchive | null>(null);
  ...
  function updateOrder(id: number, field: "client" | "address" | "comuna" | "phone" | "product" | "quantity" | "unitPrice" | "payment" | "status" | "time" | "note", value: string) { setDraft((current) => current ? { ...current, orders: current.orders.map((order) => { if (order.id !== id) return order; const item = getOrderItems(order)[0]; const nextItem = field === "product" ? { ...item, product: value } : field === "quantity" ? { ...item, quantity: Number(value) } : field === "unitPrice" ? { ...item, unitPrice: Number(value) } : item; return { ...order, client: field === "client" ? value : order.client, address: field === "address" ? value : order.address, comuna: field === "comuna" ? value : order.comuna, phone: field === "phone" ? value : order.phone, payment: field === "payment" ? value as PaymentMethod : order.payment, status: field === "status" ? value as OrderStatus : order.status, time: field === "time" ? value : order.time, note: field === "note" ? value : order.note, items: [nextItem], product: nextItem.product, quantity: nextItem.quantity, total: nextItem.quantity * nextItem.unitPrice }; }) } : current); }
  function updateExpense(index: number, field: "name" | "value", value: string) { setDraft((current) => current ? { ...current, expenses: current.expenses.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: field === "value" ? Number(value) : value } : item) } : current); }
  function addExpense() { setDraft((current) => current ? { ...current, expenses: [...current.expenses, { name: "", value: 0 }] } : current); }
  function updateDate(value: string) { setDraft((current) => current ? { ...current, date: value } : current); }
  if (!container) return null;
  return createPortal(<section className="archived-report-editor"><div className="archived-report-heading"><div><p className="section-kicker">EDICIÓN MANUAL</p><h3>Reportes guardados</h3></div><span>Corrige pedidos y gastos archivados</span></div><div className="archived-report-list">{archives.slice().reverse().map((archive) => { const totals = archiveTotals(archive); return <div key={archive.id}><b>{archive.date}</b><span>{archive.orders.length} pedidos · Ventas {money(totals.sales)} · Gastos {money(totals.expenses)} · Total {money(totals.net)}</span><button type="button" onClick={() => setDraft(JSON.parse(JSON.stringify(archive)) as DailyArchive)}>Editar</button></div>; })}</div>{draft && <div className="archive-edit-modal"><div className="archive-edit-card"><button type="button" className="daily-report-close" onClick={() => setDraft(null)}>×</button><p className="section-kicker">REPORTE {draft.date}</p><h3>Editar reporte guardado</h3><label className="archive-edit-date">Fecha del reporte<input type="date" value={draft.date} onChange={(event) => updateDate(event.target.value)} /></label><div className="archive-edit-orders">{draft.orders.map((order) => { const item = getOrderItems(order)[0]; return <div className="archive-edit-row" key={order.id}><label>Cliente<input value={order.client} onChange={(event) => updateOrder(order.id, "client", event.target.value)} /></label><label>Cantidad<input type="number" min="1" value={item.quantity} onChange={(event) => updateOrder(order.id, "quantity", event.target.value)} /></label><label>Precio<input type="number" min="0" value={item.unitPrice} onChange={(event) => updateOrder(order.id, "unitPrice", event.target.value)} /></label></div>; })}</div><h4>Gastos</h4>{draft.expenses.map((item, index) => <div className="archive-edit-expense" key={index}><input placeholder="Nombre del gasto" value={item.name} onChange={(event) => updateExpense(index, "name", event.target.value)} /><input aria-label="Monto del gasto" type="number" min="0" value={item.value} onChange={(event) => updateExpense(index, "value", event.target.value)} /></div>)}<button className="archive-add-expense" type="button" onClick={addExpense}>+ Agregar gasto</button><button className="submit-order" type="button" onClick={() => { onUpdate(draft); setDraft(null); }}>Guardar cambios</button></div></div>}</section>, container);
}
...
function AddArchivedOrder({ archives, clients, inventory, onUpdate }: { archives: DailyArchive[]; clients: Client[]; inventory: InventoryItem[]; onUpdate: (archive: DailyArchive) => void }) {
  const [selected, setSelected] = useState<DailyArchive | null>(null);
  const [form, setForm] = useState({ client: "", address: "", comuna: "", phone: "", product: "Recarga 20 litros", quantity: 1, unitPrice: 0, payment: "Efectivo" as PaymentMethod, status: "Entregado" as OrderStatus, time: "Pendiente", note: "" });
  const [extraItems, setExtraItems] = useState<OrderItem[]>([]);
  useEffect(() => { if (!selected) return; const frame = requestAnimationFrame(() => { const formElement = document.querySelector<HTMLFormElement>(".archive-add-form"); if (!formElement || formElement.querySelector(".saved-client-picker")) return; const label = document.createElement("label"); label.className = "saved-client-picker"; label.textContent = "Elegir cliente guardado"; const select = document.createElement("select"); select.innerHTML = `<option value="">Seleccionar cliente...</option>${clients.map((client) => `<option value="${client.phone}">${client.name} · ${client.phone}</option>`).join("")}`; select.addEventListener("change", () => selectClient(select.value)); label.append(select); formElement.insertBefore(label, formElement.querySelector(".archive-full-grid")); }); return () => cancelAnimationFrame(frame); }, [selected, clients]);
  useEffect(() => { if (!selected) return; const frame = requestAnimationFrame(() => { const formElement = document.querySelector<HTMLFormElement>(".archive-add-form"); if (!formElement || formElement.querySelector(".archived-products-picker")) return; const section = document.createElement("div"); section.className = "archived-products-picker"; section.innerHTML = `<b>Productos adicionales</b><div><label>Producto<select><option value="">Agregar producto del inventario...</option>${inventory.filter((item) => item.category === "Productos").map((item) => `<option value="${item.id}">${item.name}</option>`).join("")}</select></label><label>Cantidad<input type="number" min="1" value="1" aria-label="Cantidad adicional"></label><label>Precio<input type="number" min="0" value="0" aria-label="Precio adicional"></label><button type="button">Agregar</button></div><small></small>`; const select = section.querySelector("select") as HTMLSelectElement; const quantityInput = section.querySelectorAll("input")[0] as HTMLInputElement; const priceInput = section.querySelectorAll("input")[1] as HTMLInputElement; const addButton = section.querySelector("button") as HTMLButtonElement; const list = section.querySelector("small") as HTMLElement; select.addEventListener("change", () => { const item = inventory.find((product) => product.id === select.value); priceInput.value = String(item?.price ?? 0); }); addButton.addEventListener("click", () => { const item = inventory.find((product) => product.id === select.value); if (!item) return; const quantity = Math.max(1, Number(quantityInput.value)); const unitPrice = Math.max(0, Number(priceInput.value)); setExtraItems((current) => { const existing = current.find((entry) => entry.product === item.name && entry.unitPrice === unitPrice); const next = existing ? current.map((entry) => entry === existing ? { ...entry, quantity: entry.quantity + quantity } : entry) : [...current, { product: item.name, quantity, unitPrice }]; list.textContent = next.map((entry) => `${entry.quantity} × ${entry.product} · $${entry.unitPrice}`).join(" · "); return next; }); select.value = ""; quantityInput.value = "1"; priceInput.value = "0"; }); formElement.insertBefore(section, formElement.querySelector(".archive-full-grid")); }); return () => cancelAnimationFrame(frame); }, [selected, inventory]);
  function update(field: keyof typeof form, value: string) { setForm((current) => ({ ...current, [field]: field === "quantity" || field === "unitPrice" ? Number(value) : value })); }
  function selectClient(phone: string) { const client = clients.find((item) => item.phone === phone); if (client) setForm((current) => ({ ...current, client: client.name, address: client.address, comuna: client.comuna, phone: client.phone })); }
  function save(event: FormEvent) { event.preventDefault(); if (!selected || !form.client.trim() || !form.product.trim() || form.quantity < 1) return; const items = [{ product: form.product.trim(), quantity: form.quantity, unitPrice: form.unitPrice }, ...extraItems]; const id = Math.max(0, ...selected.orders.map((order) => Number(order.id) || 0)) + 1; const order: Order = { id, client: form.client.trim(), address: form.address.trim(), comuna: form.comuna.trim(), phone: form.phone.trim(), product: items[0].product, quantity: items[0].quantity, total: items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0), payment: form.payment, status: form.status, time: form.time.trim() || "Pendiente", note: form.note.trim(), items }; onUpdate({ ...selected, orders: [...selected.orders, order] }); setSelected(null); setExtraItems([]); setForm({ client: "", address: "", comuna: "", phone: "", product: "Recarga 20 litros", quantity: 1, unitPrice: 0, payment: "Efectivo", status: "Entregado", time: "Pendiente", note: "" }); }
  return <section className="archived-add-order"><div className="archived-report-heading"><div><p className="section-kicker">REGISTRO MANUAL</p><h3>Agregar pedido olvidado</h3></div><span>Incorpóralo al día correcto</span></div><div className="archived-report-list">{archives.slice().reverse().map((archive) => <div key={archive.id}><b>{archive.date}</b><span>{archive.orders.length} pedidos</span><button type="button" onClick={() => setSelected(archive)}>Agregar pedido</button></div>)}</div>{selected && <div className="archive-edit-modal"><form className="archive-edit-card archive-add-form" onSubmit={save}><button type="button" className="daily-report-close" onClick={() => setSelected(null)}>×</button><p className="section-kicker">REPORTE {selected.date}</p><h3>Agregar pedido olvidado</h3><div className="archive-full-grid"><label>Cliente<input required value={form.client} onChange={(event) => update("client", event.target.value)} /></label><label>Dirección<input value={form.address} onChange={(event) => update("address", event.target.value)} /></label><label>Comuna<input value={form.comuna} onChange={(event) => update("comuna", event.target.value)} /></label><label>Teléfono<input value={form.phone} onChange={(event) => update("phone", event.target.value)} /></label><label>Producto<input required value={form.product} onChange={(event) => update("product", event.target.value)} /></label><label>Cantidad<input required type="number" min="1" value={form.quantity} onChange={(event) => update("quantity", event.target.value)} /></label><label>Precio<input required type="number" min="0" value={form.unitPrice} onChange={(event) => update("unitPrice", event.target.value)} /></label><label>Pago<select value={form.payment} onChange={(event) => update("payment", event.target.value)}><option>Efectivo</option><option>Transferencia</option><option>Tarjeta</option></select></label><label>Estado<select value={form.status} onChange={(event) => update("status", event.target.value)}><option>Nuevo</option><option>En ruta</option><option>Entregado</option></select></label><label>Hora<input value={form.time} onChange={(event) => update("time", event.target.value)} /></label><label className="archive-edit-wide">Nota<textarea value={form.note} onChange={(event) => update("note", event.target.value)} /></label></div><button className="submit-order" type="submit">Guardar pedido</button></form></div>}</section>;
}

function FullArchivedReportEditor({ archives, onUpdate }: { archives: DailyArchive[]; onUpdate: (archive: DailyArchive) => void }) {
  const [draft, setDraft] = useState<DailyArchive | null>(null);
  function changeOrder(id: number, field: keyof Order, value: string) { setDraft((current) => current ? { ...current, orders: current.orders.map((order) => order.id !== id ? order : { ...order, [field]: field === "quantity" || field === "total" ? Number(value) : field === "payment" ? value as PaymentMethod : field === "status" ? value as OrderStatus : value }) } : current); }
  function changeItem(id: number, field: "product" | "quantity" | "unitPrice", value: string) { setDraft((current) => current ? { ...current, orders: current.orders.map((order) => { if (order.id !== id) return order; const item = getOrderItems(order)[0]; const nextItem = { ...item, [field]: field === "product" ? value : Number(value) }; return { ...order, items: [nextItem], product: nextItem.product, quantity: nextItem.quantity, total: nextItem.quantity * nextItem.unitPrice }; }) } : current); }
  function changeExpense(index: number, field: "name" | "value", value: string) { setDraft((current) => current ? { ...current, expenses: current.expenses.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: field === "value" ? Number(value) : value } : item) } : current); }
  return <>{archives.slice().reverse().map((archive) => <button className="full-report-edit-link" type="button" key={archive.id} onClick={() => setDraft(JSON.parse(JSON.stringify(archive)) as DailyArchive)}>Editar todos los datos · {archive.date}</button>)}{draft && <div className="archive-edit-modal"><div className="archive-edit-card full-archive-edit"><button type="button" className="daily-report-close" onClick={() => setDraft(null)}>×</button><p className="section-kicker">EDICIÓN COMPLETA</p><h3>Editar todos los datos del reporte</h3><label className="archive-edit-date">Fecha<input type="date" value={draft.date} onChange={(event) => setDraft({ ...draft, date: event.target.value })} /></label>{draft.orders.map((order) => { const item = getOrderItems(order)[0]; return <div className="archive-full-order" key={order.id}><strong>Pedido #{order.id}</strong><div className="archive-full-grid"><label>Cliente<input value={order.client} onChange={(event) => changeOrder(order.id, "client", event.target.value)} /></label><label>Dirección<input value={order.address} onChange={(event) => changeOrder(order.id, "address", event.target.value)} /></label><label>Comuna<input value={order.comuna} onChange={(event) => changeOrder(order.id, "comuna", event.target.value)} /></label><label>Teléfono<input value={order.phone} onChange={(event) => changeOrder(order.id, "phone", event.target.value)} /></label><label>Producto<input value={item.product} onChange={(event) => changeItem(order.id, "product", event.target.value)} /></label><label>Cantidad<input type="number" min="1" value={item.quantity} onChange={(event) => changeItem(order.id, "quantity", event.target.value)} /></label><label>Precio<input type="number" min="0" value={item.unitPrice} onChange={(event) => changeItem(order.id, "unitPrice", event.target.value)} /></label><label>Pago<select value={order.payment} onChange={(event) => changeOrder(order.id, "payment", event.target.value)}><option>Efectivo</option><option>Transferencia</option><option>Tarjeta</option></select></label><label>Estado<select value={order.status} onChange={(event) => changeOrder(order.id, "status", event.target.value)}><option>Nuevo</option><option>En ruta</option><option>Entregado</option></select></label><label>Hora<input value={order.time} onChange={(event) => changeOrder(order.id, "time", event.target.value)} /></label><label className="archive-edit-wide">Nota<textarea value={order.note ?? ""} onChange={(event) => changeOrder(order.id, "note", event.target.value)} /></label></div></div>; })}<h4>Gastos</h4>{draft.expenses.map((item, index) => <div className="archive-edit-expense" key={`${item.name}-${index}`}><input value={item.name} onChange={(event) => changeExpense(index, "name", event.target.value)} /><input type="number" min="0" value={item.value} onChange={(event) => changeExpense(index, "value", event.target.value)} /></div>)}<button className="submit-order" type="button" onClick={() => { onUpdate(draft); setDraft(null); }}>Guardar todos los cambios</button></div></div>}</>;
}
...
export function App() {
  ...
  function addInventoryItem(event: FormEvent) {
   ...
   function enableInventoryProduct(id: string) { setInventory((items) => items.map((item) => item.id === id ? { ...item, availableForSale: true } : item)); }
   function toggleUnlimitedProduct(id: string) { setInventory((items) => items.map((item) => { if (item.id !== id) return item; const unlimited = !isUnlimitedProduct(item.name); if (unlimited) unlimitedProducts.add(item.name); else unlimitedProducts.delete(item.name); return { ...item, unlimited }; })); }
  ...
  return <div className={`water-app ${activeSection === "Repartidor" ? "rider-mode" : ""}`}>
    ...
    <main className="workspace" id="top">
      ...
      {(activeSection === "Pedidos" || activeSection === "Reparto") && <div className="page-layout">
        ...
        {activeSection === "Pedidos" && <section className="panel current-orders"><div className="panel-heading"><div><p className="section-kicker">PEDIDOS ACTIVOS</p><h2>Pedidos agregados</h2></div><span>{orders.filter((order) => order.status !== "Entregado").length} pendientes</span></div>{orders.filter((order) => order.status !== "Entregado").length ? <div>{orders.filter((order) => order.status !== "Entregado").map((order) => <article key={order.id}><div><b>{order.client}</b><span>{getOrderItems(order).map((item) => `${item.quantity} × ${item.product}`).join(" · ")}</span><small>{order.address}, {order.comuna}</small></div><strong>{money(order.total)}</strong><button type="button" onClick={() => void cancelOrder(order.id)}>Cancelar y borrar</button></article>)}</div> : <p className="empty-cart">No hay pedidos activos.</p>}</section>}
