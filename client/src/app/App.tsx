...
function AddArchivedOrder({ archives, clients, onUpdate }: { archives: DailyArchive[]; clients: Client[]; onUpdate: (archive: DailyArchive) => void }) {
  ...
  function save(event: FormEvent) { event.preventDefault(); if (!selected || !form.client.trim() || !form.product.trim() || form.quantity < 1) return; const id = Math.max(0, ...selected.orders.map((order) => Number(order.id) || 0)) + 1; const order: Order = { id, client: form.client.trim(), address: form.address.trim(), comuna: form.comuna.trim(), phone: form.phone.trim(), product: form.product.trim(), quantity: form.quantity, total: form.quantity * form.unitPrice, payment: form.payment, status: form.status, time: form.time.trim() || "Pendiente", note: form.note.trim(), items: [{ product: form.product.trim(), quantity: form.quantity, unitPrice: form.unitPrice }] }; onUpdate({ ...selected, orders: [...selected.orders, order] }); setSelected(null); setForm({ client: "", address: "", comuna: "", phone: "", product: "Recarga 20 litros", quantity: 1, unitPrice: 0, payment: "Efectivo", status: "Entregado", time: "Pendiente", note: "" }); }
...
export function App() {
  ...
  return <div className={`water-app ${activeSection === "Repartidor" ? "rider-mode" : ""}`}>
    ...
    <main className="workspace" id="top">
      ...
      {activeSection === "Reportes" && <AddArchivedOrder archives={dailyArchives} clients={clientList} onUpdate={updateDailyArchive} />}
