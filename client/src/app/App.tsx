...
function AddArchivedOrder({ archives, clients, inventory, onUpdate }: { archives: DailyArchive[]; clients: Client[]; inventory: InventoryItem[]; onUpdate: (archive: DailyArchive) => void }) {
  ...
  useEffect(() => { if (!selected) return; const frame = requestAnimationFrame(() => { const formElement = document.querySelector<HTMLFormElement>(".archive-add-form"); if (!formElement || formElement.querySelector(".archived-products-picker")) return; const section = document.createElement("div"); section.className = "archived-products-picker"; section.innerHTML = `<b>Productos adicionales</b><div><select><option value="">Agregar producto del inventario...</option>${inventory.filter((item) => item.category === "Productos").map((item) => `<option value="${item.id}">${item.name}</option>`).join("")}</select><input type="number" min="1" value="1" aria-label="Cantidad adicional"><button type="button">Agregar</button></div><small></small>`; const select = section.querySelector("select") as HTMLSelectElement; const quantityInput = section.querySelector("input") as HTMLInputElement; const addButton = section.querySelector("button") as HTMLButtonElement; const list = section.querySelector("small") as HTMLElement; addButton.addEventListener("click", () => { const item = inventory.find((product) => product.id === select.value); if (!item) return; const quantity = Math.max(1, Number(quantityInput.value)); setExtraItems((current) => { const existing = current.find((entry) => entry.product === item.name); const next = existing ? current.map((entry) => entry.product === item.name ? { ...entry, quantity: entry.quantity + quantity } : entry) : [...current, { product: item.name, quantity, unitPrice: item.price ?? 0 }]; list.textContent = next.map((entry) => `${entry.quantity} × ${entry.product}`).join(" · "); return next; }); select.value = ""; quantityInput.value = "1"; }); formElement.insertBefore(section, formElement.querySelector(".archive-full-grid")); }); return () => cancelAnimationFrame(frame); }, [selected, inventory]);
