...
function MonthlyCloseControl({ closures, dailyArchives, onClose, onDailyClose }: { closures: MonthlyClosure[]; dailyArchives: DailyArchive[]; onClose: () => void; onDailyClose: () => void }) {
  ...
  async function shareArchive(archive: DailyArchive) { const text = `Reporte De La Roca ${archive.date}\n${archive.orders.length} pedidos\nVentas: ${money(archive.orders.reduce((sum, order) => sum + order.total, 0))}\nGastos: ${money(archive.expenses.reduce((sum, item) => sum + item.value, 0))}`; if (navigator.share) await navigator.share({ title: `Reporte ${archive.date}`, text }); else { await navigator.clipboard?.writeText(text); window.alert("Resumen copiado. Puedes pegarlo en WhatsApp."); } }
...
function AddArchivedOrder({ archives, clients, inventory, onUpdate }: { archives: DailyArchive[]; clients: Client[]; inventory: InventoryItem[]; onUpdate: (archive: DailyArchive) => void }) {
  ...
  useEffect(() => { if (!selected) return; const frame = requestAnimationFrame(() => { const formElement = document.querySelector<HTMLFormElement>(".archive-add-form"); if (!formElement || formElement.querySelector(".archived-products-picker")) return; const section = document.createElement("div"); section.className = "archived-products-picker"; section.innerHTML = `<b>Productos adicionales</b><div><select><option value="">Agregar producto del inventario...</option>${inventory.filter((item) => item.category === "Productos").map((item) => `<option value="${item.id}">${item.name}</option>`).join("")}</select><input type="number" min="1" value="1" placeholder="Cantidad" aria-label="Cantidad adicional"><input type="number" min="0" value="0" placeholder="Precio" aria-label="Precio adicional"><button type="button">Agregar</button></div><small></small>`; const select = section.querySelector("select") as HTMLSelectElement; const quantityInput = section.querySelectorAll("input")[0] as HTMLInputElement; const priceInput = section.querySelectorAll("input")[1] as HTMLInputElement; const addButton = section.querySelector("button") as HTMLButtonElement; const list = section.querySelector("small") as HTMLElement; select.addEventListener("change", () => { const item = inventory.find((product) => product.id === select.value); priceInput.value = String(item?.price ?? 0); }); addButton.addEventListener("click", () => { const item = inventory.find((product) => product.id === select.value); if (!item) return; const quantity = Math.max(1, Number(quantityInput.value)); const unitPrice = Math.max(0, Number(priceInput.value)); setExtraItems((current) => { const existing = current.find((entry) => entry.product === item.name && entry.unitPrice === unitPrice); const next = existing ? current.map((entry) => entry === existing ? { ...entry, quantity: entry.quantity + quantity } : entry) : [...current, { product: item.name, quantity, unitPrice }]; list.textContent = next.map((entry) => `${entry.quantity} × ${entry.product} · $${entry.unitPrice}`).join(" · "); return next; }); select.value = ""; quantityInput.value = "1"; priceInput.value = "0"; }); formElement.insertBefore(section, formElement.querySelector(".archive-full-grid")); }); return () => cancelAnimationFrame(frame); }, [selected, inventory]);
