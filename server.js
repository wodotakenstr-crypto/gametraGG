...
function orderUpdateEmailHtml(order, eyebrow, title, message, amount) {
  const orderUrl = `https://gametradegg.com/orders?order=${encodeURIComponent(order.id)}`;
...
async function sendAdminPaymentEmail(order) {
  ...
  await mailTransport.sendMail({
    ...
    text: `Se registró un pago manual de ${amount} USDT TRC20 para ${order.id}. Referencia: ${order.paymentTxId}. Revísalo y confírmalo manualmente en Administración: https://gametradegg.com/admin`,
...
app.post("/api/orders/:orderId/paypal/create", async (request, response) => {
  ...
  try {
    ...
    const returnUrl = externalUrl(request, `/paypal-return?order=${encodeURIComponent(participant.order.id)}&state=${encodeURIComponent(state)}`);
